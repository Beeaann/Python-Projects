...
class MyTabView(ctk.CTkTabview):
    ...
    def chaseTab(self):
        ...
        while isDateEntered and isInitialPayment:
            ...
            balance = self.chasePaymentVar.get()
            ...
            self.amortization_table_maker(passedDate, chasePaymentAmount, 
                                          ...
                                          chaseSifExpirationDate, balance)
    ...
    def citiOtherTab(self):
        ...
        while isDateEntered and isInitialPayment:
            ...
            balance = self.citiOtherPaymentVar.get()
            citiOtherDownPayment = self.citiOtherDownPaymentVar.get()    
            self.amortization_table_maker(passedDate, citiOtherPaymentAmount, newDate, citiOtherFinalPayment, citiOtherNumPayments, citiOtherDownPayment, citiOtherSifExpirationDate, balance)
    ...
    def citiMacyTab(self):
        ...
        while isDateEntered == True and isLessThan12 == True and isInitialPayment == True:
            ...
            try:
                ...
                print("textbox clear error - ", error)
            balance = self.citiMacysPaymentVar.get()
            citiMacysDownPayment = self.citiMacysDownPaymentVar.get()     
            self.amortization_table_maker(passedDate, citiMacysPaymentAmount, newDate, citiMacysFinalPayment, citiMacysNumPayments, citiMacysDownPayment, citiMacysSifExpirationDate, balance)
    ...
    def targetTab(self):
        ...
        while isDateEntered and isInitialPayment:
            ...
            try:
                ...
                print("textbox clear error - ", error)
            balance = self.targetPaymentVar.get()
            targetDownPayment = self.targetDownPaymentVar.get() 
            self.amortization_table_maker(passedDate, targetPaymentAmount, newDate, targetFinalPayment, targetNumPayments, targetDownPayment, targetSifExpirationDate, balance)
    ...
    def bqTab(self):
        ...
        while isDateEntered and isInitialPayment:
            ...
            try:
                ...
                print("textbox clear error - ", error)
            balance = self.bqPaymentVar.get()
            bqDownPayment = self.bqDownPaymentVar.get() 
            self.amortization_table_maker(passedDate, bqPaymentAmount, newDate, bqFinalPayment, bqNumPayments, bqDownPayment, bqSifExpirationDate, balance)
    ...
    def cavalryTab(self):
        ...
        while isDateEntered and isInitialPayment:
            ...
            balance = self.cavalryPaymentVar.get()
            self.amortization_table_maker(passedDate, cavalryPaymentAmount, newDate, cavalryFinalPayment, cavalryNumPayments, cavalryDownPayment, cavalrySifExpirationDate, balance)
    ...
    def amortization_table_maker(self, first_payment_date, first_monthly_payment, last_payment_date, 
                                 ...
                                 sif_expiration_date, accountBalance):
    ...
    def addToTable(self):
        ...
        if pressedCounter == 0:
            textbox.configure(state="normal")  
            textbox.insert(ctk.END, f"\nRESPONSIBLE: {user}")
            textbox.insert(ctk.END, f"\nBALANCE: ${self.accountBalance:.2f}")
            ...
            if self.downPayment == 0:
                ...
                textbox.insert(ctk.END, (f"\nPAYMENT TERMS: Starting on {self.formatted_first_date} a payment of ${self.downPayment}, on {self.startdifPayment} a payment of\n"
                                        f"${self.last_monthly_payment:.2f} , and then on {self.startMonthPayments} {self.totalPayments - 2} payments of ${self.first_monthly_payment:.2f}"))
